...
describe('AuthController refresh token (e2e)', () => {
  ...
  describe('Validation', () => {
    it('Should return 401 when request is not send refresh token', async () => {
      const result = await request(app.getHttpServer())
        .post(ROUTER_END_TO_END_TESTER.Auth.refreshToken)
        .send();
  ...
  describe('Business', () => {
    it('Should return 201 when refresh token ok', async () => {
